...
io
  ...
  .on('connection', function (socket) {
    ...
    socket.on('adduser', function (username) {
      ...
      let defaultRoom = rooms[0]; // установим комнату по умолчанию
      ...
      socket.json.emit('updatechat', { name: 'System', msg: `<i>Вы попали в комнату ${defaultRoom}</i>`}); // выполним событие updatechat для сообщения куда он попал
    ...
    socket.on('switchroom', function (newroom) {
      ...
      socket.json.emit('updatechat', { name: 'System', msg: `<i>Вы попали в комнату ${newroom}</i>` });
      // сообщаем в старую комнату, что пользователь ее покинул
      socket.broadcast.to(oldRoom).json.emit('updatechat', { name: 'System', msg: `<i>${users[socket.id].name} покинул комнату</i>` });
      // сообщаем в новую комнату, что зашел новый пользователь
      socket.broadcast.to(newroom).json.emit('updatechat', { name: 'System', msg: `<i>${users[socket.id].name} присоединился к комнате</i>` });
    ...
    socket.on('message', function (data) {
      ...
      socket.json.emit('updatechat', { name: users[socket.id].name, msg: data });
      // сообщаем всем пользователям комнаты, что написал пользователь
      socket.broadcast.to(users[socket.id].room).json.emit('updatechat', { name: users[socket.id].name, msg: data });
    });
